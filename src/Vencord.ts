...
async function runUpdateCheck() {
    ...
    try {
        ...
        showNotice(
            `A new version of ${BRAND_NAME} is available!`,
            "View Update",
...
function initTrayIpc() {
    ...
    VencordNative.tray.onCheckUpdates(async () => {
        try {
            ...
            if (isOutdated) {
                showNotice(`A ${BRAND_NAME} update is available!`, "View Update", () => openSettingsTabModal(UpdaterTab!));
            } else {
    ...
    VencordNative.tray.onRepair(async () => {
        try {
            ...
            UpdateLogger.error(`Failed to repair ${BRAND_NAME}`, err);
        }
    });
    ...
    if (IS_DEV) {
        ...
        if (pendingPatches.length)
            PMLogger.warn(
                ...
                `You are seeing this warning because this is a Development build of ${BRAND_NAME}.`,
