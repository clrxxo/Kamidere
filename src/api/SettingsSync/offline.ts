...
import { BRAND_NAME, BRAND_NAME_LOWER } from "@shared/branding";
...
export async function downloadSettingsBackup(type: BackupType = "all", { minify }: { minify?: boolean; } = {}) {
    try {
        ...
        const filename = `${BRAND_NAME_LOWER}-${type}-backup-${moment().format("YYYY-MM-DD")}.json`;
...
export async function uploadSettingsBackup(type: BackupType = "all", showToast = true): Promise<void> {
    if (IS_DISCORD_DESKTOP) {
        const [file] = await DiscordNative.fileManager.openFiles({
            filters: [
                { name: `${BRAND_NAME} Settings Backup`, extensions: ["json"] },
                { name: "all", extensions: ["*"] }
            ]
        });
